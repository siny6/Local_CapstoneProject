...
public abstract class Weapon : MonoBehaviour
{
    ...
    // //===================================
    // // 현재 설정된 타겟이 유효한지 검사 - 타겟 리스트에서 유효하지 않은 타겟을 제거하고 타겟을 재탐색함 
    // //===================================
    // void CheckTargetList()
    // {
    //     int invalidNum = 0; //유효하지 않은 적의 수 판별

    //     // 타겟리스트 검사
    //     for(int i = list_targets.Count-1;i>=0;i--)
    //     {
    //         Transform currTarget = list_targets[i]; // 검사할 타겟

    //         // 해당 타겟이 죽었거나, 거리를 벗어났는지 판별    - 사망 판정, 또는 공격 불가 판정은 enemy 스크립트 완성후에 추가 
    //         if (Vector2.Distance(currTarget.position, transform.position) > range )
    //         {
    //             list_targets.Remove(currTarget);  // 리스트에서 제거
    //             invalidNum++;
    //         }
    //     }
    //     // 유효하지 않은 타겟이 있다면, 타겟 재 탐색
    //     if (invalidNum >0 )
    //     {
    //        SearchTarget(invalidNum); 
    //     }
    // }
    ...
    public void Attack()
    {
        ...
        {
            //lastAttackTime = GameManager.instance.gameTime;   // 마지막 공격 시간 갱신
